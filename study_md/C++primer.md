...
{
public:
    Foo& operator=(const Foo&)&; //限定左值对象才可使用
    Foo& operator=(const Foo&) &&; 	//限定右值对象cai'e'k
...
class Derived : public base
{
    void foo2(Base& b, Derived& d)
    {
        n++;
        this->n++;
        //b.n++; //error 只能通过this访问基类的protect变量
        d.n++;
    }
}
```
...
- capture list    : 捕获列表，lamda所在函数中的局部变量定义
    - `[]`：不捕获任何变量。
    - `[&]`：以引用的方式捕获所有外部变量。
    - `[=]`：以值的方式捕获所有外部变量。
    - `[x]`：以值的方式捕获变量x。
...
#else
#define assert(condition) // do assert //
#endif
```

- debug模式下会进行输出
- 当条件不成立，会终止程序并输出错误的出处，包括函数和文件
...
int main()
{
    int * ptr = new(nothrow) int; //new 将不抛出异常
    return 0;
}
```
